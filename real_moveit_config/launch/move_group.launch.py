...
def _write_real_flag(context, *args, **kwargs):
    path = os.environ.get("ROBOT_ENV_FILE", MODE_FILE_DEFAULT)
    os.makedirs(os.path.dirname(path), exist_ok=True)
    with open(path, "w") as f:
        f.write("real\n")
    return [LogInfo(msg=f"[env-flag] wrote '{path}' = real")]
...
def generate_launch_description():
    ...
    # Write the flag first, then start Move Group
    return LaunchDescription([
        OpaqueFunction(function=_write_real_flag),
        move_group_node,
    ])
